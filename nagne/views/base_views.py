from django.core.paginator import Paginator
from django.shortcuts import render, get_object_or_404
from django.db.models import Q
from ..models import Assignment

def index(request):
    page = request.GET.get('page','1')
    kw = request.GET.get('kw', '')  # 검색어
    assignment_list = Assignment.objects.order_by('-create_date')
    if kw:
        assignment_list = assignment_list.filter(
            Q(subject__icontains=kw) |  # 제목 검색
            Q(content__icontains=kw) |  # 내용 검색
            Q(answer__content__icontains=kw) |  # 답변 내용 검색
            Q(author__username__icontains=kw) |  # 질문 글쓴이 검색
            Q(answer__author__username__icontains=kw)  # 답변 글쓴이 검색
        ).distinct()
    paginator = Paginator(assignment_list, 10)
    page_obj = paginator.get_page(page)
    context = {'assignment_list': page_obj, 'page': page, 'kw': kw}
    return render(request, 'nagne/assignment_list.html', context)


def detail(request, assignment_id):
    assignment = get_object_or_404(Assignment, pk=assignment_id)
    context = {'assignment': assignment}
    return render(request, 'nagne/assignment_detail.html', context)